...
###################################################################
# Compute RR & CI from the IPW model: using baseline data
###################################################################

source("EffectEstimate_IPW_BL.R")




DATA = read.csv('data_for_BLeffect.csv')
fname <- paste0(outDir, 'IP_BL_boots_RR_LL_UL.csv') 


boots <- data.frame(i = 1:5, row.name = NA, RR = NA, SE =NA, LL = NA, UL = NA)
...
for (treatment in c('smoking','sbp','cholesterol')){
  ...
  for (treatType in c('level', 'medication')){
    ...
    data_sub = getSubData(DATA,treatment,treatTpe)
    ...
    d <- copy(data_sub)
    d <- getTreatW(data_sub,treatment)
